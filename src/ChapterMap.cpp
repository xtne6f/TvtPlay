...
bool CChapterMap::Open(LPCTSTR path)
{
    ...
    if (!rv || rv >= _countof(tmpPath) - 8) return false;

    if (!::PathFileExists(tmpPath) ||
        !::lstrcpyn(m_path, tmpPath, _countof(m_path) - 8) ||
...
bool CChapterMap::Save() const
{
    ...
    for (const_iterator it = begin(); it != end(); ++it) {
        ASSERT(it->first >= 0);
        p += ::wsprintf(p, TEXT("%dc%s-"), it->first, it->second.val);
    }
    ::lstrcat(p, TEXT("c"));
...
bool CChapterMap::InsertCommand(LPCTSTR p)
{
    ...
    if (!::StrCmpNI(p, TEXT("c-"), 2)) {
        ...
        for (;;) {
            ...
            CHAPTER ch;
            if (!::StrToIntEx(p, STIF_DEFAULT, &ch.first) || ch.first < 0) break;
