...
#define INI_MUTEX_NAME_W	L"TVTest"
...
namespace TVTest
{
	...
	bool CIniFile::Open(LPCWSTR pszFileName,UINT Flags)
	{
		...
		String MutexName;
		MutexName=INI_MUTEX_NAME_W L"_Ini_Mutex_";
	...
	bool CIniFile::Close()
	{
		...
		if ((m_OpenFlags&OPEN_WRITE)!=0 && m_hFile!=INVALID_HANDLE_VALUE) {
			...
			for (std::list<CSectionData>::const_iterator itrSection=m_SectionList.begin();itrSection!=m_SectionList.end();itrSection++) {
				...
				for (std::list<CEntry>::const_iterator itrValue=itrSection->Entries.begin();itrValue!=itrSection->Entries.end();itrValue++) {
	...
	bool CIniFile::SelectSection(LPCWSTR pszSection)
	{
		...
		std::list<CSectionData>::iterator i=FindSection(pszSection);
	...
	bool CIniFile::GetValue(LPCWSTR pszName,String *pValue)
	{
		...
		std::list<CEntry>::iterator itrValue=FindValue(pSection->Entries,pszName);
	...
	bool CIniFile::SetValue(LPCWSTR pszName,LPCWSTR pszValue)
	{
		...
		if (itrValue==pSection->Entries.end()) {
			...
			for (std::list<CEntry>::reverse_iterator i=pSection->Entries.rbegin();i!=pSection->Entries.rend();i++) {
				if (!i->Name.empty()) {
	...
	bool CIniFile::IsValueExists(LPCWSTR pszName)
	{
		...
		std::list<CEntry>::const_iterator itrValue=FindValue(pSection->Entries,pszName);
	...
	bool CIniFile::GetSectionEntries(LPCWSTR pszSection,std::vector<CEntry> *pEntries)
	{
		...
		std::list<CSectionData>::iterator itrSection=FindSection(pszSection);
		if (itrSection==m_SectionList.end())
			return false;

		for (std::list<CEntry>::iterator i=itrSection->Entries.begin();i!=itrSection->Entries.end();i++) {
			if (!i->Name.empty()) {
	...
	CIniFile::SectionList::iterator CIniFile::FindSection(LPCWSTR pszName)
	{
		...
		for (std::list<CSectionData>::iterator i=m_SectionList.begin();i!=m_SectionList.end();i++) {
	...
	CIniFile::SectionList::const_iterator CIniFile::FindSection(LPCWSTR pszName) const
	{
		...
		for (std::list<CSectionData>::const_iterator i=m_SectionList.begin();i!=m_SectionList.end();i++) {
			if (StringUtility::CompareNoCase(i->SectionName,pszName)==0)
				return i;
		}
	...
	{
		for (std::list<CEntry>::iterator i=Entries.begin();i!=Entries.end();i++) {
	...
	{
		for (std::list<CEntry>::const_iterator i=Entries.begin();i!=Entries.end();i++) {
			if (!i->Name.empty() && StringUtility::CompareNoCase(i->Name,pszName)==0)
				return i;
		}
	...
	CIniFile::CSectionData *CIniFile::GetCurSection()
	{
		...
		if (m_pCurSection!=nullptr) {
			std::list<CSectionData>::iterator itrSection=FindSection(m_Section.c_str());
			if (itrSection==m_SectionList.end())
				return nullptr;
