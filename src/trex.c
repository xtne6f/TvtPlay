...
static int trex_element(TRex *exp)
{
...

	{
		...
		switch(*exp->_p){
			...
			case '{':
				exp->_p++;
				if(*exp->_p < '0' || '9' < *exp->_p) trex_error(exp,_SC("number expected"));
...
{
	switch(cclass) {
	case 'a': return scisalpha(c)?TRex_True:TRex_False;
	case 'A': return !scisalpha(c)?TRex_True:TRex_False;
	case 'w': return (scisalnum(c) || c == '_')?TRex_True:TRex_False;
	case 'W': return (!scisalnum(c) && c != '_')?TRex_True:TRex_False;
	case 's': return scisspace(c)?TRex_True:TRex_False;
	case 'S': return !scisspace(c)?TRex_True:TRex_False;
	case 'd': return scisdigit(c)?TRex_True:TRex_False;
	case 'D': return !scisdigit(c)?TRex_True:TRex_False;
	case 'x': return scisxdigit(c)?TRex_True:TRex_False;
	case 'X': return !scisxdigit(c)?TRex_True:TRex_False;
	case 'c': return sciscntrl(c)?TRex_True:TRex_False;
	case 'C': return !sciscntrl(c)?TRex_True:TRex_False;
	case 'p': return scispunct(c)?TRex_True:TRex_False;
	case 'P': return !scispunct(c)?TRex_True:TRex_False;
	case 'l': return scislower(c)?TRex_True:TRex_False;
	case 'u': return scisupper(c)?TRex_True:TRex_False;
	}
...
static const TRexChar *trex_matchnode(TRex* exp,TRexNode *node,const TRexChar *str,TRexNode *next)
{
	...
	case OP_WB:
		if(str == exp->_bol && !scisspace(*str)
		 || (str == exp->_eol && !scisspace(*(str-1)))
		 || (!scisspace(*str) && scisspace(*(str+1)))
		 || (scisspace(*str) && !scisspace(*(str+1))) ) {
