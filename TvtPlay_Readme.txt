TVTest TvtPlay Plugin ver.1.4(人柱版) + BonDriver_Pipe.dll
...
れません。問題があればとりあえずver.0.9r7に戻ってください(設定ファイルはそのまま
で大丈夫です)。
チャプター機能をテスト実装中です。興味ある方は後述「チャプター機能について」を参
照のうえ、慎重に利用してください。
...
Button[00-17]
    ...
    # コマンドはキー割り当て順に Open,OpenPupup,ListPopup,Close,Prev,SeekToBgn,
    # SeekToEnd,SeekToPrev,SeekToNext,AddChapter,RepeatChapter,SkipXChapter,
    # Loop,Pause,Stretch,StretchRe,Seek[A-Z],Stretch[A-Z]【ver.1.4～】
