TVTest TvtPlay Plugin ver.0.8 + BonDriver_Pipe.dll
                              + TvtAudioStretchFilter.ax(人柱版)
...
・通常版: Visual C++ 2005 SP1再頒布可能パッケージ(TVTestが起動するなら入ってる)
・x64版:  Visual C++ 2010 再頒布可能パッケージ (x64)

■以前のバージョン(ver.0.4～ver.0.7)からの移行
・TvtPlay.tvtpを置きかえてください。設定ファイルはそのまま引き継げます。
・ver.0.8では早送りボタン2個が追加されました。アイコン画像をカスタマイズしている
  方は画像の編集が必要になるかもしれません。
・ver.0.8での変更点は早送りとフォルダ簡易表示の機能追加のみです。これらを利用し
  ない場合はver.0.7を置きかえる必要はありません。
...
ル末尾へシーク、早送り200%/50%、です。なお、ファイルを開くボタンを右クリックする
と、TVTestの録画フォルダにある.tsファイルを簡易表示します。コントロール中央はシ
ークバーになっていて、左クリックで任意の位置にシーク、右クリックで再生を一時停止
/再開します。コントロール右側には再生位置および総再生時間が表示されます。

早送り機能を利用する方は音声フィルタの設定が必要です。同梱のDirectshowフィルタ
TvtAudioStretchFilter.axを適当なフォルダに入れ、コマンドプロンプトから管理者権限
で regsvr32 {フィルタのパス} と入力してください(この辺は「DirectShowフィルタ 登
録」あたりでググってください)。無事登録されていれば、TVTestの 設定->再生->音声フ
ィルタ にこのフィルタが現れるので、これを選択してください。

*** 以下、必要に応じてお読みください ***

■早送り再生について
早送り再生は音声を伸び縮みさせることで実現しています。動画をスキップさせているわ
けではないので、早送りの速度に応じてシステムの負荷が高まります。また、あまり高速
に再生する(=高負荷にする)と数秒～数十秒間TVTestが固まるようです。しばらく放って
おけば戻りますが、あまり早送りの速度を上げすぎないように注意してください。
また、早送りに使用するTvtAudioStretchFilterですが、早送り再生時以外は何も処理を
...
導入にはVisual C++ 2005 SP1 再頒布可能パッケージ(x64版はVisual C++ 2010 再頒布可
能パッケージ (x64))が必要です。x64版はファイル名を必ず"BonDriver_Pipe.dll"にリネ
ームしてください。

■TVTest起動オプションについて
ver.0.7以降では/tvtpudpおよび/tvtpipeオプションが追加されました。各オプションの
意味はつぎのようになります。
...
Stretch[A-D]【ver.0.8～】
    早送り再生の速度を、等速にたいする比率(パーセント)で指定する
    # [=25]～[=800]まで設定可能です。
Button[00-15]
    ...
    # PauseとLoopとStretch[A-D]コマンドは右隣のアイコンも使います。
...
また、TVTest ver.0.7.21からソースコードを流用しています。特に以下のファイ
